...
/// artwork's center.
class AlertOptInPanel extends StatelessWidget {
  ...
  Future<void> _accept(BuildContext context) async {
    final bool granted = await alertHub.askPermission();
    if (!granted) {
      await store.writeInviteResumeTs(_cooldownTarget());
    }
  ...
  Widget build(BuildContext context) {
    ...
    // Landscape: horizontal center on the full screen — Accept above
    // Skip, both the same pill style, no opacity trick (§12 pitfalls).
    // Portrait: same layout, wider buttons.

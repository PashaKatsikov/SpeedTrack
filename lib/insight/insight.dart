...
import '../cfg/insight_env.dart';

/// Crash-safe facade over Microsoft Clarity. Session replay captures
/// the native Flutter surface; these calls add funnel signals that
/// answer "where did the user drop off?".
///
/// Every public method is guarded — a Clarity failure must NEVER
/// propagate into the gray-flow state machine.
class Insight {
  const Insight._();
  ...
  static void identify(String? aid,
  ...
  /// Sets the screen label AND emits a stable per-screen event.
  ...
  /// `last_screen` tag so drop-off filtering works instantly.
